...
export class AddCardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.cardDetails.id = '';
    this.cardDetails.clientId = this.loggedInUser.id;
    this.cardDetails.clientName = this.loggedInUser.name;
    this.cardDetails.email = this.loggedInUser.email;
  }
  ...
  async saveCard() {
    if (this.validateCardDetails()) {
      try {
        ...
        await this.card
          .addPaymentMethod(this.cardDetails)
          .then(async (result: any) => {
            if (this.cardDetails.DefaultPaymentMethod) {
              from(
                this.card.getPaymentMethodData(this.loggedInUser.id)
                ...
                this.paymentMethodsList.forEach((payment) => {
                  ...
                  console.log(
                    `this.cardDetails.clientName : ${this.cardDetails.clientName}`
                  );
                  console.log(
                  ...
                  if (
                    payment.cardNumber === this.cardDetails.cardNumber &&
